...
async function initMsal(){
 const clientId=localStorage.getItem("finance.clientId")||"";$("clientId").value=clientId;$("redirectUri").textContent=location.href.split("#")[0].split("?")[0];
 if(!clientId)return;
 msal=new PublicClientApplication({auth:{clientId,authority:"https://login.microsoftonline.com/common",redirectUri:location.href.split("#")[0].split("?")[0]},cache:{cacheLocation:"localStorage"}});
...
async function sign(){
 if(account){await msal.logoutRedirect({account});return}
 if(!msal){show("settings");$("connectionStatus").textContent="Add your Microsoft Application (client) ID in Settings first.";return}
 await msal.loginRedirect({scopes:SCOPES});
}
async function token(){
 try{return (await msal.acquireTokenSilent({account,scopes:SCOPES})).accessToken}
 catch{return (await msal.acquireTokenPopup({account,scopes:SCOPES})).accessToken}
}
